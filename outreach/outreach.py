...
SAMPLE_URL = "https://www.instagram.com/beautifullife.movie/"  # ← サンプル完成後に変更
...
{salon_name}さまの丁寧なグルーミングへのこだわりに共感し、ご連絡させていただきました。

私どもは、ペット事業者さまのお店の世界観をシネマティックな映像で記録・発信するサービスを提供しております。

月1回の訪問で、トリミングシーンや店舗の雰囲気を高品質な映像として制作し、SNS素材として納品いたします。

先日、提携先のサロンで制作したサンプルがございます。
ぜひご覧ください：{sample_url}

貴サロンのブランド価値をさらに高める映像を一緒に作れればと考えております。
...
{salon_name}さんのSNSをいつも拝見しており、わんちゃんへの愛情が伝わってご連絡しました。

私たちは、ペットサロンさま向けに月1回の訪問でSNS投稿素材（動画・写真）をまとめて制作・納品するサービスをしています。

撮影・編集はすべてお任せください。オーナーさまは投稿するだけです。

提携先サロンのサンプル動画はこちら：{sample_url}

導入サロンさまには、Beautiful Lifeのサイト・SNSからも優先的にご紹介しています。
まずはお話だけでも大歓迎です。お気軽にご返信ください！
...
{salon_name}さんのこだわりのあるお仕事ぶり、いつも素敵だなと思っています。

私たちは、月1回サロンにお伺いして、わんちゃんたちの様子や店舗の雰囲気を映像として残すサービスをしています。

「SNSを更新したいけど撮影・編集が大変…」というお悩みを、まるごと解決します。

サンプルはこちらからご覧いただけます：{sample_url}

小規模サロンさまも大歓迎です。まずはお気軽にお声がけください！
...
{salon_name}さまの取り組みに注目しており、ぜひご提案したくご連絡いたしました。

私どもは、ペット事業者さまのSNSコンテンツ制作・ブランドムービー・採用動画を月額制で提供するサービスを展開しております。

複数店舗での導入実績もあり、各店舗の個性を映像で表現することで、集客・採用・ブランディングに効果をいただいています。

まずはサンプルをご覧いただけますか：{sample_url}

詳細なご提案も可能ですので、ぜひ一度お話しする機会をいただければ幸いです。
...
先日お送りしたサンプル動画、ご覧いただけましたでしょうか？

「まずはお話だけでも」という形でも大歓迎ですので、
お気軽にご返信いただけると嬉しいです。
